...
def test_run_map_briefing_renders_readable_packet_without_raw_source_ids(tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        "if 'analyst_adjudication_v1' in prompt and 'analyst_decision_model_v1' not in prompt:\n"
        "    ids = sorted(set(re.findall(r'\"evidence_item_id\": \"([^\" ]+)\"', prompt)))\n"
        "    payload = {'schema_id': 'analyst_adjudication_v1', 'decision_question': 'What should a decision-maker conclude?', 'rows': [{'evidence_item_id': evidence_id, 'memo_use': 'load_bearing_primary_support', 'answer_relation': 'supports_answer', 'importance_rank': index + 1, 'rationale': 'Fixture adjudication row.', 'source_ids': [], 'quantity_values': []} for index, evidence_id in enumerate(ids)], 'overall_rationale': 'Fixture adjudication.'}\n"
        "    print(json.dumps(payload)); raise SystemExit\n"
        "if 'analyst_decision_model_v1' in prompt:\n"
        "    ids = sorted(set(re.findall(r'\"evidence_item_id\": \"([^\" ]+)\"', prompt)))\n"
        "    payload = {'schema_id': 'analyst_decision_model_v1', 'decision_question': 'What should a decision-maker conclude?', 'direct_answer': 'Use the case as a crux source, not a settled conclusion.', 'confidence': 'medium', 'overall_rationale': 'The fixture evidence is narrow but decision-relevant.', 'evidence_groups': [{'group_id': 'fixture_group', 'proposition': 'The case turns on whether priors or likelihood updates explain the disagreement.', 'memo_role': 'load_bearing_primary_support', 'answer_relation': 'supports_answer', 'importance_rank': 1, 'covered_evidence_item_ids': ids, 'rationale': 'Fixture grouped evidence.'}], 'evidence_dispositions': [{'evidence_item_id': evidence_id, 'disposition': 'foreground', 'group_id': 'fixture_group', 'rationale': 'Fixture foreground.'} for evidence_id in ids], 'quantitative_anchors': [], 'what_would_change_the_answer': ['New evidence separated priors from likelihoods.'], 'decision_logic': {'bounded_bottom_line': 'Use the case as a crux source, not a settled conclusion.', 'support_summary': 'The case turns on whether priors or likelihood updates explain the disagreement.', 'strongest_counterweight': '', 'counterweight_weighting': '', 'reconciled_cruxes': ['Whether priors or likelihood updates explain disagreement.'], 'scope_boundaries': [], 'practical_implications': ['Use the source to frame the crux rather than settle the decision.'], 'do_not_overstate': ['Do not imply the map settles the full case.']}, 'argument_plan': [{'step_id': 'answer', 'writing_goal': 'State the bounded read.', 'required_points': ['The source frames the crux.'], 'evidence_item_ids': ids}]}\n"
        "    print(json.dumps(payload)); raise SystemExit\n"
        "if 'analyst_packet_refinement_v1' in prompt:\n"
        "    payload = {'schema_id': 'analyst_packet_refinement_v1', 'decision_question': 'What should a decision-maker conclude?', 'direct_answer': 'Use the case as a crux source, not a settled conclusion.', 'answer_rationale': 'The fixture evidence identifies the crux without settling all sources.', 'decision_logic': {'bounded_bottom_line': 'Use the case as a crux source, not a settled conclusion.', 'support_summary': 'The case turns on priors versus likelihood updates.', 'strongest_counterweight': '', 'counterweight_weighting': '', 'reconciled_cruxes': ['Whether priors or likelihood updates explain disagreement.'], 'scope_boundaries': [], 'practical_implications': ['Treat the source as crux-framing evidence.'], 'do_not_overstate': ['Do not imply the full case is settled.']}, 'warning_obligations': [], 'argument_plan': []}\n"
        "    print(json.dumps(payload)); raise SystemExit\n"
...
def test_synthesize_map_briefing_cli_fails_closed_for_unready_map(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        "if 'analyst_adjudication_v1' in prompt and 'analyst_decision_model_v1' not in prompt:\n"
        "    ids = sorted(set(re.findall(r'\"evidence_item_id\": \"([^\" ]+)\"', prompt)))\n"
        "    payload = {'schema_id': 'analyst_adjudication_v1', 'decision_question': 'What follows?', 'rows': [{'evidence_item_id': evidence_id, 'memo_use': 'load_bearing_primary_support', 'answer_relation': 'supports_answer', 'importance_rank': index + 1, 'rationale': 'Fixture adjudication row.', 'source_ids': [], 'quantity_values': []} for index, evidence_id in enumerate(ids)], 'overall_rationale': 'Fixture adjudication.'}\n"
        "    print(json.dumps(payload)); raise SystemExit\n"
        "if 'analyst_decision_model_v1' in prompt:\n"
        "    ids = sorted(set(re.findall(r'\"evidence_item_id\": \"([^\" ]+)\"', prompt)))\n"
        "    payload = {'schema_id': 'analyst_decision_model_v1', 'decision_question': 'What follows?', 'direct_answer': 'Alpha matters for the decision.', 'confidence': 'low', 'overall_rationale': 'The fixture has one support row.', 'evidence_groups': [{'group_id': 'fixture_group', 'proposition': 'Alpha matters for the decision.', 'memo_role': 'load_bearing_primary_support', 'answer_relation': 'supports_answer', 'importance_rank': 1, 'covered_evidence_item_ids': ids, 'rationale': 'Fixture grouped evidence.'}], 'evidence_dispositions': [{'evidence_item_id': evidence_id, 'disposition': 'foreground', 'group_id': 'fixture_group', 'rationale': 'Fixture foreground.'} for evidence_id in ids], 'quantitative_anchors': [], 'what_would_change_the_answer': [], 'decision_logic': {'bounded_bottom_line': 'Alpha matters for the decision.', 'support_summary': 'Alpha matters for the decision.', 'strongest_counterweight': '', 'counterweight_weighting': '', 'reconciled_cruxes': [], 'scope_boundaries': [], 'practical_implications': ['Use Alpha as the main decision consideration.'], 'do_not_overstate': []}, 'argument_plan': []}\n"
        "    print(json.dumps(payload)); raise SystemExit\n"
        "if 'analyst_packet_refinement_v1' in prompt:\n"
        "    payload = {'schema_id': 'analyst_packet_refinement_v1', 'decision_question': 'What follows?', 'direct_answer': 'Alpha matters for the decision.', 'answer_rationale': 'The fixture has one support row.', 'decision_logic': {'bounded_bottom_line': 'Alpha matters for the decision.', 'support_summary': 'Alpha matters for the decision.', 'strongest_counterweight': '', 'counterweight_weighting': '', 'reconciled_cruxes': [], 'scope_boundaries': [], 'practical_implications': ['Use Alpha as the main decision consideration.'], 'do_not_overstate': []}, 'warning_obligations': [], 'argument_plan': []}\n"
        "    print(json.dumps(payload)); raise SystemExit\n"
        "if 'You are writing one section of a source-grounded decision memo' in prompt:\n"
        "    match = re.search(r'exactly(?: with)?: ## (.+)', prompt)\n"
        "    heading = match.group(1).strip() if match else 'Why This Is the Best Current Read'\n"
...
def test_semantic_staged_brief_cli_runs_full_path_and_fails_closed_when_not_published(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        "elif 'analyst_adjudication_v1' in prompt and 'analyst_decision_model_v1' not in prompt:\n"
        "    ids = sorted(set(re.findall(r'\"evidence_item_id\": \"([^\" ]+)\"', prompt)))\n"
        "    payload = {'schema_id': 'analyst_adjudication_v1', 'decision_question': 'Should this demo decision rely on Alpha or Gamma?', 'rows': [{'evidence_item_id': evidence_id, 'memo_use': 'load_bearing_primary_support', 'answer_relation': 'supports_answer', 'importance_rank': index + 1, 'rationale': 'Fixture adjudication row.', 'source_ids': [], 'quantity_values': []} for index, evidence_id in enumerate(ids)], 'overall_rationale': 'Fixture adjudication.'}\n"
        "elif 'analyst_decision_model_v1' in prompt:\n"
        "    ids = sorted(set(re.findall(r'\"evidence_item_id\": \"([^\" ]+)\"', prompt)))\n"
        "    payload = {'schema_id': 'analyst_decision_model_v1', 'decision_question': 'Should this demo decision rely on Alpha or Gamma?', 'direct_answer': 'Treat Gamma as the crux while using Alpha as support.', 'confidence': 'medium', 'overall_rationale': 'Alpha supports the decision and Gamma changes how it should be read.', 'evidence_groups': [{'group_id': 'fixture_group', 'proposition': 'Alpha supports the decision while Gamma is the key crux.', 'memo_role': 'load_bearing_primary_support', 'answer_relation': 'supports_answer', 'importance_rank': 1, 'covered_evidence_item_ids': ids, 'rationale': 'Fixture grouped evidence.'}], 'evidence_dispositions': [{'evidence_item_id': evidence_id, 'disposition': 'foreground', 'group_id': 'fixture_group', 'rationale': 'Fixture foreground.'} for evidence_id in ids], 'quantitative_anchors': [], 'what_would_change_the_answer': ['Gamma changes whether Alpha should guide the decision.'], 'decision_logic': {'bounded_bottom_line': 'Treat Gamma as the crux while using Alpha as support.', 'support_summary': 'Alpha supports the decision.', 'strongest_counterweight': 'Gamma is the key crux.', 'counterweight_weighting': 'Gamma bounds how directly Alpha should guide the decision.', 'reconciled_cruxes': ['Gamma changes whether Alpha should guide the decision.'], 'scope_boundaries': [], 'practical_implications': ['Review Gamma before relying on Alpha.'], 'do_not_overstate': []}, 'argument_plan': []}\n"
        "elif 'analyst_packet_refinement_v1' in prompt:\n"
        "    payload = {'schema_id': 'analyst_packet_refinement_v1', 'decision_question': 'Should this demo decision rely on Alpha or Gamma?', 'direct_answer': 'Treat Gamma as the crux while using Alpha as support.', 'answer_rationale': 'Alpha supports the decision and Gamma changes how it should be read.', 'decision_logic': {'bounded_bottom_line': 'Treat Gamma as the crux while using Alpha as support.', 'support_summary': 'Alpha supports the decision.', 'strongest_counterweight': 'Gamma is the key crux.', 'counterweight_weighting': 'Gamma bounds how directly Alpha should guide the decision.', 'reconciled_cruxes': ['Gamma changes whether Alpha should guide the decision.'], 'scope_boundaries': [], 'practical_implications': ['Review Gamma before relying on Alpha.'], 'do_not_overstate': []}, 'warning_obligations': [], 'argument_plan': []}\n"
