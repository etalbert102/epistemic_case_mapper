...
def test_staged_semantic_map_assigns_ids_and_rejects_bad_chunk_claims(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        "            {'claim': 'Alpha line.', 'entailed_by_excerpt': 'yes', 'question_relevance': 'direct', 'scope_flags': ['none'], 'decision_importance': 'high', 'why_it_matters': 'It bears on the decision.', 'supporting_quotes': [{'quote': 'Alpha line.', 'line_hint': 'lines 1-1'}], 'quantities': [], 'scope_conditions': []},\n"
        "            {'claim': 'Wrong quote should be rejected.', 'entailed_by_excerpt': 'yes', 'question_relevance': 'direct', 'scope_flags': ['none'], 'decision_importance': 'medium', 'why_it_matters': 'It should not survive validation.', 'supporting_quotes': [{'quote': 'Missing source quote.', 'line_hint': 'lines 99-99'}], 'quantities': [], 'scope_conditions': []}\n"
        "        ], 'excluded_as_not_decision_relevant': []}\n"
        "    else:\n"
        "        payload = {'source_id': 'demo_case_doc_b', 'source_bottom_line': 'Gamma is relevant.', 'canonical_claims': [\n"
        "            {'claim': 'Gamma line.', 'entailed_by_excerpt': 'yes', 'question_relevance': 'direct', 'scope_flags': ['none'], 'decision_importance': 'high', 'why_it_matters': 'It changes how Alpha is read.', 'supporting_quotes': [{'quote': 'Gamma line.', 'line_hint': 'lines 1-1'}], 'quantities': [], 'scope_conditions': []}\n"
        "        ], 'excluded_as_not_decision_relevant': []}\n"
...
def test_staged_semantic_map_reports_backend_errors_without_claim_fallbacks(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        f"if {WHOLE_DOC_CLAIM_PROMPT_VERSION!r} in prompt:\n"
        "    payload = {'source_id': 'demo_case_doc_b', 'source_bottom_line': 'Gamma is relevant.', 'canonical_claims': [\n"
        "        {'claim': 'Gamma line.', 'entailed_by_excerpt': 'yes', 'question_relevance': 'direct', 'scope_flags': ['none'], 'decision_importance': 'high', 'why_it_matters': 'It is the surviving source-card claim.', 'supporting_quotes': [{'quote': 'Gamma line.', 'line_hint': 'lines 1-1'}], 'quantities': [], 'scope_conditions': []}\n"
        "    ], 'excluded_as_not_decision_relevant': []}\n"
        f"elif {RELATION_PROMPT_VERSION!r} in prompt:\n"
...
def test_staged_semantic_map_can_accept_quality_repair(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        "        {'claim': claim, 'entailed_by_excerpt': 'yes', 'question_relevance': 'direct', 'scope_flags': ['none'], 'decision_importance': 'high', 'why_it_matters': 'It supports the initialized package question.', 'supporting_quotes': [{'quote': quote, 'line_hint': 'lines 1-1'}], 'quantities': [], 'scope_conditions': []}\n"
        "    ], 'excluded_as_not_decision_relevant': []}\n"
        f"elif {RELATION_PROMPT_VERSION!r} in prompt:\n"
        "    payload = {}\n"
        "else:\n"
...
def test_staged_semantic_map_records_chunk_budget(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        "        {'claim': quote, 'entailed_by_excerpt': 'yes', 'question_relevance': 'direct', 'scope_flags': ['none'], 'decision_importance': 'high', 'why_it_matters': 'It should enter the map despite chunk budgeting.', 'supporting_quotes': [{'quote': quote, 'line_hint': 'lines 1-1'}], 'quantities': [], 'scope_conditions': []}\n"
        "    ], 'excluded_as_not_decision_relevant': []}\n"
        "elif 'preparing claims for relation-building' in prompt:\n"
        "    ids = sorted(set(re.findall(r'[a-zA-Z0-9_]+_c[0-9]{3}', prompt)))\n"
        "    payload = {'roles': [{'claim_id': claim_id, 'decision_edge_role': 'outcome_finding', 'role_confidence': 'high', 'rationale': 'Fixture model role for relation selection.'} for claim_id in ids]}\n"
...
def test_staged_semantic_map_batches_relation_pairs(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_model.write_text(
        ...
        "        {'claim': quote, 'entailed_by_excerpt': 'yes', 'question_relevance': 'direct', 'scope_flags': ['none'], 'decision_importance': 'high', 'why_it_matters': 'It should enter relation batching.', 'supporting_quotes': [{'quote': quote, 'line_hint': 'lines 1-1'}], 'quantities': [], 'scope_conditions': []}\n"
        "    ], 'excluded_as_not_decision_relevant': []}\n"
        "elif 'preparing claims for relation-building' in prompt:\n"
        "    ids = sorted(set(re.findall(r'[a-zA-Z0-9_]+_c[0-9]{3}', prompt)))\n"
        "    payload = {'roles': [{'claim_id': claim_id, 'decision_edge_role': 'outcome_finding', 'role_confidence': 'high', 'rationale': 'Fixture model role for relation selection.'} for claim_id in ids]}\n"
