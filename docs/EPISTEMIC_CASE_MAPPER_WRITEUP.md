...
The result is a runnable reference prototype with source manifests, structured maps, Markdown and JSON exports, comparison baselines, structure-loss and erosion-audit surfaces, reviewer packets, validation tools, resumable processing stages, and a static inspection UI. A newer synthesis layer can also turn mapped evidence into a decision-oriented memo through an explicit intermediate decision model. That layer has substantially improved traceability and retention, but the repository’s own evaluations show that final prose quality remains uneven and does not consistently outperform a strong Deep Research-style answer.
...
Recent decision-briefing evaluations add a second kind of evidence. The pipeline can now carry an explicit decision model, bounded quantities, counterweights, scope conditions, and mandatory retention obligations into a memo. Saved and live runs show improved structural reliability and a meaningful uplift over the short COVID flat baseline. The eggs case is more sobering: the newer pipeline is easier to audit and loses fewer required elements, but its final memo still trails a strong Deep Research baseline in answer crispness, quantitative integration, subgroup detail, and prose quality. That result usefully narrows the claim. The mapper’s demonstrated advantage is inspectable reasoning and controlled handoff, not universal superiority at final-answer writing.
...
The `prompt` backend writes all prompts and scaffolds without calling a model. Live execution can use a local command backend or an Ollama model. The CLI also exposes package validation, JSON export, UI generation, baselines, synthesis, review-packet generation, unseen-case quality checks, and automated stress evaluation.
...
- **Reproducibility:** checked-in examples, prompt-only execution, resumable stages, and package gates make the workflow critiqueable.
...
- `docs/FLF_BEFORE_AFTER_COMPARISON.md` — compact before/after examples.
...
- `docs/EVIDENCE_AND_LIMITATIONS.md` — full risk register.
- `docs/CASE_INIT_AND_MODEL_BACKENDS.md` — runnable workflow.
