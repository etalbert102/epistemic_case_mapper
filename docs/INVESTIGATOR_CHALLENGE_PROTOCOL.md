...
Scores are descriptive. The judge packet links to raw prompts, raw retrieved responses, and score records so the aggregate numbers do not need to be trusted on their own.
...
The LHC slice adds the already acquired but held-out source `cern_lhc_current_page` from `docs/NEW_SOURCE_UPDATE_DEMO.md`. The runner writes:
