...
- Failure lineage is genuinely broken. [`_memo_ready_synthesis_failed()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_final_outputs.py#L343) returns failure only for three recognized statuses after seeing `accepted != true`; an unknown or newly added rejected status therefore continues through repair and presentation. [`build_final_decision_readiness_report()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_readiness.py#L46) receives no synthesis, repair, or polish lineage at all. The saved run demonstrates the result: [synthesis was blocked](<https://github.com/etalbert102/epistemic_case_mapper/blob/main/artifacts/fresh_eggs_20260718_000324/briefing/memo_ready_synthesis_report.json>), while [final readiness said `decision_ready`](<https://github.com/etalbert102/epistemic_case_mapper/blob/main/artifacts/fresh_eggs_20260718_000324/briefing/final_decision_readiness_report.json>) and [memo quality scored 100](<https://github.com/etalbert102/epistemic_case_mapper/blob/main/artifacts/fresh_eggs_20260718_000324/briefing/memo_quality_report.json>). Workstream 1 directly addresses this.

- Immutable result identity is the correct fix for the wrong estimate/CI pairing in [BRIEFING.md](https://github.com/etalbert102/epistemic_case_mapper/blob/main/artifacts/fresh_eggs_20260718_000324/briefing/BRIEFING.md#L8). Current extraction turns quantities into independent values in [`_quantity_rows_for_claim()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_quantities.py#L436). Even the artifact named `source_quantity_tuples.json` is misleading: [`_quantity_tuples_for_unit()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/staged_semantic_evidence_units.py#L188) creates one “tuple” per loose value, not one estimate–interval–endpoint result. The plan correctly treats this as a data-model defect.

- The evidence-universe diagnosis is also precise. [`ensure_reader_memo_metadata()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_memo_metadata.py#L11) falls back from cited sources to every entry in `source_display_names`. Meanwhile [`_source_titles_for_region()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/cli_semantic.py#L691) returns all case sources, not the region’s required sources. That is exactly how a seven-source active packet produced a twelve-source list.

- Evidence budgeting and simpler section jobs are directionally right. The saved analyst packet considered 15 of 16 groups foreground while still reporting `ready`. Current deterministic budgeting in [`apply_obligation_budget()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_decision_diagnosticity.py#L87) only limits `must_include`; it does not force the analyst to make a genuinely sparse foreground selection.

- The plan correctly rejects deterministic keyword semantics in principle. The evaluation’s examples remain in production code, including [`_typed_fields()` and `_lexically_supported()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/staged_semantic_evidence_units.py#L155), [`_decision_factor()` and `_direction()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/decision_argument_artifacts.py#L578), and role inference in the decision packet. [SUMMARY_OF_FINDINGS.md](https://github.com/etalbert102/epistemic_case_mapper/blob/main/artifacts/fresh_eggs_20260718_000324/briefing/SUMMARY_OF_FINDINGS.md#L21) shows the resulting polarity errors.

## Major architectural gaps

### 1. `AnalystDecisionContract` risks becoming another packet

The repo already has an object close to the proposed contract: [`AnalystDecisionModel`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_analyst_schemas.py#L346). It already contains the direct answer, confidence, evidence groups and dispositions, quantity decisions, source hierarchy, update conditions, decision logic, and argument plan.

That model is currently projected through:

1. [`global_decision_model`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_global_decision_model.py#L59)
2. [`decision_writer_packet`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_decision_writer_packet.py#L184)
3. `memo_ready_packet`
4. [`canonical_decision_writer_packet`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_canonical_decision_writer_packet.py#L43)
...
This matters because the current prompt still calls several structures “controlling” or “primary” simultaneously in [`build_canonical_decision_writer_packet_synthesis_prompt()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_memo_ready_prompt.py#L133).

### 2. The plan assumes the analyst contract itself is trustworthy

The saved [analyst_decision_model.json](<https://github.com/etalbert102/epistemic_case_mapper/blob/main/artifacts/fresh_eggs_20260718_000324/briefing/analyst_decision_model.json>) already contains unsupported or overstated judgments such as “safety ceiling” and an invented LDL monitoring recommendation. Making that artifact authoritative would faithfully propagate bad judgment.
...
The existing whole-document model already extracts structured quantity objects through [`source_card_json_schema()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/staged_semantic_whole_doc.py#L255), but that schema permits independent quantity values. The plan should explicitly replace that schema with result-level objects and migrate or delete the existing `source_quantity_tuples` representation. Creating a parallel `result_tuples.json` would leave two competing meanings of “tuple.”
...
The staged mapper knows `region_id`, required sources, selected chunks, and omitted chunks. That information is recorded in the map run summary, but [`run_map_briefing()`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_pipeline.py#L81) receives only the map, question, and source lookup dictionaries.
...
The existing human-authored [source independence notes](https://github.com/etalbert102/epistemic_case_mapper/blob/main/data/cases/eggs/metadata/source_independence.md#L1) are not consumed by the active pipeline. The plan needs to say whether such metadata is authoritative input, model context, or only a hint.
...
- Relation-value ablation. The evaluation recommends comparing no graph, current graph, and decision-targeted relations. The current [relation_value_report.json](<https://github.com/etalbert102/epistemic_case_mapper/blob/main/artifacts/fresh_eggs_20260718_000324/briefing/relation_value_report.json>) calls the graph useful largely from structural properties. The plan should either make relations prove downstream value or remove them from memo obligations.
...
One execution detail also needs correction: the plan names `map_briefing_analyst_decision_model.py`, which does not exist. The live implementation is centered on [`map_briefing_analyst_decision_modeling.py`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_analyst_decision_modeling.py#L56), with orchestration in [`map_briefing_decision_packet_stage.py`](https://github.com/etalbert102/epistemic_case_mapper/blob/main/src/epistemic_case_mapper/map_briefing_decision_packet_stage.py#L132). The latter, plus `cli_semantic.py`, should be explicit owned integration points.
