...
def _synthesis_group_sections(packet: dict[str, Any]) -> tuple[str, ...]:
    return (
...
def _foreground_sections() -> tuple[str, ...]:
    return (
        "primary_reasoning_chain",
        "main_counterweights",
        "decision_cruxes",
...
def _ids_for_roles(rows: list[dict[str, Any]], roles: set[str], *, limit: int) -> list[str]:
    return [
        str(row.get("evidence_item_id") or "")
        for row in rows
...
def _applicability_limits(memo_use: str, ledger_row: dict[str, Any], adjudication_row: dict[str, Any]) -> list[str]:
    limits = []
...
def _why_this_read(groups: list[dict[str, Any]]) -> str:
    primary = _first_group_text(groups, "load_bearing_primary_support")
...
def _must_not_overstate(groups: list[dict[str, Any]]) -> list[str]:
    lines = []
    for group in groups:
