...
def build_canonical_decision_writer_packet(
    ...
    *,
    ...
    language_contracts = _evidence_language_contracts(packet, interface)
...
def _evidence_language_contracts(packet: dict[str, Any], interface: dict[str, Any]) -> list[dict[str, Any]]:
    rows = []
...
def _evidence_language_contract(
    row: dict[str, Any],
    *,
    source_appraisals: dict[str, Any],
    index: int,
) -> dict[str, Any]:
    appraisal = _dict(row.get("source_appraisal"))
    if not appraisal:
        appraisal = _merged_source_appraisal_for_row(row, source_appraisals)
    ...
    allowed_verbs = _allowed_verbs(allowed, design=design, warnings=warnings)
    avoid_verbs = _avoid_verbs(allowed, warnings=warnings)
    source_labels = _string_list(row.get("source_labels")) or _string_list(row.get("source_label"))
    ...
    return _drop_empty(
        {
            ...
            "allowed_language": allowed_verbs,
            "avoid_language": avoid_verbs,
