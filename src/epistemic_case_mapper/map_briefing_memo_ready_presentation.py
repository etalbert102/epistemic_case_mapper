...
def run_memo_ready_presentation_normalization(
    ...
    *,
    ...
    if next_memo != normalized:
        ...
        normalized = next_memo
    next_memo = _align_inline_citations(normalized, packet)
    if next_memo != normalized:
...
def _align_inline_citations(memo: str, packet: dict[str, Any]) -> str:
    entries = _canonical_source_entries(packet)
    if not entries:
        return str(memo or "")
    display_lookup = _citation_display_lookup(entries)
    ...
    for line in lines:
        ...
        if (
            ...
            aligned_lines.append(line)
            continue
        aligned_lines.append(_align_citation_line(line, display_lookup, source_by_display, role_by_source))
    return "\n".join(aligned_lines)
